  ...
  return (
    ...
    >
      ...
      {EDGES.slice(0, 5).map(([a, b], i) => {
        ...
        const nb = nodeByKey(b);
        return (
          <circle key={`p-${i}`} r="2.5" fill="#e8a33d">
            <animateMotion
              dur={`${4 + i * 0.6}s`}
      ...
      {NODES.map((n) => (
        <g key={n.key}>
          <circle cx={n.x} cy={n.y} r="3.5" fill="#e8a33d" />
          <text
            x={n.x}
            y={n.y - 12}
            textAnchor="middle"
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(60% 50% at 80% 15%, rgba(232,163,61,0.10), transparent 60%)",
        }}
      />
      ...
      <motion.div
        ...
        animate="show"
        variants={container}
        className="relative z-10 mx-auto grid w-full max-w-6xl grid-cols-1 items-center gap-14 lg:grid-cols-[1.3fr_0.9fr]"
      >
        <div>
